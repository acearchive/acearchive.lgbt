...
for (const form of gatewayUrlForms) {
    ...
    const updateUrlCopyValue = () => {
        copyButton.setAttribute("value", `https://replayweb.page/?source=ipfs://${cidInput.value}?filename=${encodeURIComponent(filenameInput.value)}#view=resources&urlSearchType=prefix&url=${encodeURIComponent(urlInput.value)}`);
    }
